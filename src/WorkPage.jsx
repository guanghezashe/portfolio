...
function WorkMedia({ project, onFormat }) {
  ...
  useEffect(() => {
    if (project.media !== "video" || !videoRef.current) return undefined;
  ...
  return (
    <img
      src={project.previewSrc || project.coverUrl || project.src}
      alt={project.title}
      loading="lazy"
      decoding="async"
      onLoad={(event) => onFormat(formatFromDimensions("image", event.currentTarget.naturalWidth, event.currentTarget.naturalHeight))}
    />
  );
}
...
function PortfolioTile({ project, onSelect }) {
  ...
  return (
    <button
      className={`portfolio-tile ${format} ${project.media === "video" ? "is-motion" : ""} ${isDetailPreview(project) ? "is-detail-preview" : ""}`}
      onClick={() => onSelect(project)}
      aria-label={`查看作品：${project.title}`}
    >
      <WorkMedia project={project} onFormat={setFormat} />
      <span className="portfolio-tile-info">
...
export default function WorkPage() {
  ...
  const visibleWork = useMemo(() => (category === allCategory ? works : works.filter((project) => project.category === category)), [category, works]);
    ...
    <>
      <header className="site-header work-header">
        <a className="brand" href="/" aria-label="回到首页">
          ...
          <span>CHEN WEIGUANG</span>
        </a>
        <nav>
          <a href="/">首页</a>
          <a className="current" href="/work">作品</a>
          <a href="/#contact">联系</a>
        </nav>
        <a className="header-contact" href="/#contact">CONTACT {"->"}</a>
      </header>
      <main className="work-page">
        <section className="work-hero page-width">
          <p className="section-number">COMPLETE ARCHIVE / 2026</p>
          <h1>作品档案</h1>
          <p>{loading ? "正在读取作品库。" : "收录全部动画与渲染作品。悬停查看信息，点击进入完整预览。"}</p>
        </section>
        <section className="work-feed-section page-width">
          <div className="work-category-bar" aria-label="作品分类">
            {categories.map((item) => (
          ...
          <div className="portfolio-masonry">
            {visibleWork.map((project) => (
              <PortfolioTile key={project.id} project={project} onSelect={setSelectedProject} />
            ))}
          </div>
        </section>
      </main>
